...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let resultAVG = await user.sequelize.query(`
      With TimeTable (N) as
        ...
        XT (x1,x2,x3,x4,x5,x6,x7,x8,x9,x10) as 
              ...
              and [DataforAnalysis].[dbo].[Dynamic_Parallelism_Tester].[Line] = '${line}'
...
router.get("/motordimmodel/:startDate/:finishDate", async (req, res) => {
  try {
    const { startDate, finishDate } = req.params;

    // Use the received startDate and finishDate in your SQL query or data retrieval logic
    let result = await user.sequelize.query(`
      SELECT DISTINCT [Model]
      FROM [DataforAnalysis].[dbo].[Dynamic_Parallelism_Tester]
      WHERE [Model] != '' AND [Date] BETWEEN :startDate AND :finishDate
      ORDER BY [Model]
    `, {
      replacements: { startDate, finishDate },
...
router.get("/motordimline/:myModel/:startDate/:finishDate", async (req, res) => {
  try {
    const { myModel, startDate, finishDate } = req.params;
    let result = await user.sequelize.query(`
...
router.get("/motordimmachine/:myLine/:myModel/:startDate/:finishDate", async (req, res) => {
  try {
    const { myLine, myModel, startDate, finishDate } = req.params;
    var line = myLine.trim();

    var result = await user.sequelize
      .query(`
        SELECT DISTINCT [Machine_no], [Line]
        FROM [DataforAnalysis].[dbo].[Dynamic_Parallelism_Tester]
        WHERE [DataforAnalysis].[dbo].[Dynamic_Parallelism_Tester].[Line] = '${myLine}'
          ...
          AND [DataforAnalysis].[dbo].[Dynamic_Parallelism_Tester].[Date] BETWEEN '${startDate}' AND '${finishDate}'
      `);
