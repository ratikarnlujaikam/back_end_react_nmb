    ...
    try {
     ...
     selectTime (T) as (select convert(varchar(10),N) as [Time] from TimeTable),		
		XT (x1,x2,x3,x4,x5,x6,x7,x8,x9,x10) as 
          (select cast(DATEPART(hour,[DataforAnalysis].[dbo].[Hipot_test].[Time]) as varchar) as [hr]
          ,case when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) < 120 
          then datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) 
          when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) >= 120 then 0 end as [C/T in sec]
          ,case when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) >= 120 
          then datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) 
          when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) < 120 then 0 end as [D/T]
          ,case when [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] < [TransportData].[dbo].[Master_matchings].[USL]
          and [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] > [TransportData].[dbo].[Master_matchings].[LSL]
          then [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] end  as [Parameter]
          ,[DataforAnalysis].[dbo].[Hipot_test].[Model] as [model]
          ...
          ,'Average' as [AVG]
          ,[DataforAnalysis].[dbo].[Hipot_test].[Barcode]
          FROM [DataforAnalysis].[dbo].[Hipot_test]
          INNER JOIN [TransportData].[dbo].[Master_matchings]
          ON [DataforAnalysis].[dbo].[Hipot_test].Model = [TransportData].[dbo].[Master_matchings].Model
          where CONVERT(DATE,[DataforAnalysis].[dbo].[Hipot_test].[Time]) = '${selectDate}'
          and [DataforAnalysis].[dbo].[Hipot_test].[Model] = '${model}'
          and [DataforAnalysis].[dbo].[Hipot_test].[Line] = '${productionline}'
          and [TransportData].[dbo].[Master_matchings].[Parameter] = '${parameter}'
          and [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] > 0 and [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] < 10
      ...
      for (let index = 0; index < selectMC.length; index++) {
       ...
       selectTime (T) as (select convert(varchar(10),N) as [Time] from TimeTable),		
		XT (x1,x2,x3,x4,x5,x6,x7,x8,x9,x10) as 
          ...
          ,[TransportData].[dbo].[Master_matchings].CL as [CL]
          ,[TransportData].[dbo].[Master_matchings].USL as [USL]
          ,[DataforAnalysis].[dbo].[Hipot_test].[Machine_no] as [MC]
          ,[DataforAnalysis].[dbo].[Hipot_test].[Barcode]
          FROM [DataforAnalysis].[dbo].[Hipot_test]
          INNER JOIN [TransportData].[dbo].[Master_matchings]
          ON [DataforAnalysis].[dbo].[Hipot_test].Model = [TransportData].[dbo].[Master_matchings].Model
          ...
          and [DataforAnalysis].[dbo].[Hipot_test].[Line] = '${productionline}'
          and [TransportData].[dbo].[Master_matchings].[Parameter] = '${parameter}'
          and [DataforAnalysis].[dbo].[Hipot_test].[Machine_no] = '${arrayMC}'
          and [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] > 0 and [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] < 10
          and [TransportData].[dbo].[Master_matchings].[createdAt] = (select max([TransportData].[dbo].[Master_matchings].[createdAt]) from [TransportData].[dbo].[Master_matchings])
          ),
          MasterRemove (M1) as (
...
FROM N3),	
	Data as(select count([Hipot_test].Barcode)as CountBarcode ,[Hipot_test].Date FROM [DataforAnalysis].[dbo].[Hipot_test]
    INNER JOIN [TransportData].[dbo].[Master_matchings]
    ON [DataforAnalysis].[dbo].[Hipot_test].Model = [TransportData].[dbo].[Master_matchings].Model 
	 where [DataforAnalysis].[dbo].[Hipot_test].[Date] BETWEEN '${startDate}' AND '${finishDate}'
    and [DataforAnalysis].[dbo].[Hipot_test].[Model] = '${model}'
    and [DataforAnalysis].[dbo].[Hipot_test].[Line] = '${productionline}'
    and [TransportData].[dbo].[Master_matchings].[Parameter] = '${parameter}'
    and [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] > 0 and [DataforAnalysis].[dbo].[Hipot_test].[${parameter}] < 10
    and [TransportData].[dbo].[Master_matchings].[createdAt] = (select max([TransportData].[dbo].[Master_matchings].[createdAt]) from [TransportData].[dbo].[Master_matchings])
	group by Date
	having count([Hipot_test].Barcode)>100)

	,XT (x1,x2,x3,x4,x5,x6,x7,x8,x9,x10) as 
    (select [DataforAnalysis].[dbo].[Hipot_test].[Date] as [Date]
    ,case when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) < 120 
    then datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) 
    when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) >= 120 then 0 end as [C/T in sec]
    ,case when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) >= 120 
    then datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) 
    when datediff(SECOND,0,cast((lag([DataforAnalysis].[dbo].[Hipot_test].[Time]) over (order by [DataforAnalysis].[dbo].[Hipot_test].[Time] desc) - [DataforAnalysis].[dbo].[Hipot_test].[Time]) as time)) < 120 then 0 end as [D/T]
    ,[DataforAnalysis].[dbo].[Hipot_test].[${parameter}]  as [Parameter]
    ,[DataforAnalysis].[dbo].[Hipot_test].[Model] as [model]
    ,[TransportData].[dbo].[Master_matchings].LSL as [LSL]
    ,[TransportData].[dbo].[Master_matchings].CL as [CL]
    ,[TransportData].[dbo].[Master_matchings].USL as [USL]
    ...
    INNER JOIN [TransportData].[dbo].[Master_matchings]
    ON [DataforAnalysis].[dbo].[Hipot_test].Model = [TransportData].[dbo].[Master_matchings].Model
	left join Data on Data.Date  = [Hipot_test].Date
    where [DataforAnalysis].[dbo].[Hipot_test].[Date] BETWEEN '${startDate}' AND '${finishDate}'
    and [DataforAnalysis].[dbo].[Hipot_test].[Model] = '${model}'
    and [DataforAnalysis].[dbo].[Hipot_test].[Line] = '${productionline}'
    and [TransportData].[dbo].[Master_matchings].[Parameter] = '${parameter}'
  ...
  Data as(select count([Hipot_test].Barcode)as CountBarcode ,[Hipot_test].Date FROM [DataforAnalysis].[dbo].[Hipot_test]
    ...
    order by d`);
        ...
        let raw_data = await user.sequelize
        .query(`with Data as(select count([Hipot_test].Barcode)as CountBarcode ,[Hipot_test].Date FROM [DataforAnalysis].[dbo].[Hipot_test]
    INNER JOIN [TransportData].[dbo].[Master_matchings]
    ON [DataforAnalysis].[dbo].[Hipot_test].Model = [TransportData].[dbo].[Master_matchings].Model 
	 where [DataforAnalysis].[dbo].[Hipot_test].[Date] BETWEEN '${startDate}' AND '${finishDate}'
    and [DataforAnalysis].[dbo].[Hipot_test].[Model] = '${model}'
...
router.get("/motorhipotmachine/:myLine/:myModel", async (req, res) => {
  try {
    ...
    let result = await user.sequelize.query(`Select distinct [Machine_no], [Line]
      FROM [DataforAnalysis].[dbo].[Hipot_test]
      where [DataforAnalysis].[dbo].[Hipot_test].[Line] = '${myLine}'
      and [DataforAnalysis].[dbo].[Hipot_test].[Model] = '${myModel}'`);
    res.json({
      result: result[0],
      api_result: "ok",
    });
