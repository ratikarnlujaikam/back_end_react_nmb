...
router.get("/MC_ERROR/:Table/:Line/:year_start/:year_end", async (req, res) => {
  try {
    var result = [[]];
    const { Table, Line,year_start,year_end} = req.params;
    const Linechaing = Line.replace('_','/');
    var resultGraph = await user1.sequelize.query(`
    DECLARE @StartMonth INT = ${year_start.split('-')[1]}; -- Replace with the desired start month
    DECLARE @StartYear INT = ${year_start.split('-')[0]}; -- Replace with the desired start year
    DECLARE @EndMonth INT = ${year_end.split('-')[1]}; -- Replace with the desired end month
    DECLARE @EndYear INT = ${year_end.split('-')[0]}; -- Replace with the desired end year
...
router.get("/MC_ERROR_NG/:Table/:Line/:year_start/:year_end", async (req, res) => {
  try {
    var result = [[]];
    const { Table, Line,year_start,year_end} = req.params;
    const Linechaing = Line.replace('_','/');
