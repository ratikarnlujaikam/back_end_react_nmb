...
router.get("/LARPP/:year/:start", async (req, res) => {
  try {
    ...
    -- Generate the list of distinct processes
    SET @pivot_columns = STUFF(
        (SELECT DISTINCT ',' + QUOTENAME([Process]) 
         FROM (
    ...
    print @pivot_columns
    -- Your PIVOT query using the dynamic pivot columns
    ...
    SELECT Hour,' + @pivot_columns + '
    FROM set1
      PIVOT (
            SUM(Qty) FOR [Process] IN (' + @pivot_columns + ')
        ) AS PivotTable;
